...
CONFIRM_RX = re.compile(
    ...
    r"|thanks for applying|submission (was )?successful", re.I,
)
...
def fill_visible_fields(page, resume: str = "", cover: str = "") -> dict:
    ...
    try:
        ...
        return {"filled": 0, "unmapped": [], "aborted": None}
    for el in handles:
        try:
            ...
            if want == FM.PREFER_NOT:
                # leave for select fuzzy later
                if typ != "file":
                    continue
            if typ == "file":
                path = resume if re.search(r"resume|cv", lab, re.I) or not cover else cover
                if re.search(r"cover", lab, re.I) and cover:
                    path = cover
...
def click_apply_or_submit(page, prefer_submit: bool = False) -> bool:
    pattern = (
        r"submit application|submit|finish|send application"
        if prefer_submit
        else r"apply now|apply for this job|apply to this job|apply without an account|start application|apply"
    )
    try:
            ...
            if looks_dead(body):
                ...
                if gate:
                    ...
                    if len(context.pages) > 1:
                        ...
                        page.bring_to_front()
                    dismiss_banners(page)
                    filled_stats = fill_visible_fields(page, resume, cover_letter)
                    if filled_stats.get("aborted"):
                        ...
                        click_apply_or_submit(page, prefer_submit=True)
                        page.wait_for_timeout(7000)
                        try:
                            body2 = page.evaluate("()=>document.body.innerText") or ""
                        except Exception:
                            body2 = ""
                        conf = confirm(page.url, body2)
                        if conf:
                            status, detail = "applied", conf
                        elif filled_stats.get("unmapped"):
                            status, detail = "needs_human", "unmapped: " + ", ".join(filled_stats["unmapped"][:5])
                        else:
